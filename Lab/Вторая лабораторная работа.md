...
### Шаг 1: Подготовка клиентов

**Tiny Core Linux** (лёгкие QEMU-узлы в GNS3):

* Скачайте образ `microcore-mininit.qcow2` (минимальная версия без графики) из [релизов на GitHub](https://github.com/grossmj/microcore-mininit/releases).
* Поместите файл в `GNS3/images/QEMU` и создайте шаблон QEMU VM.

Подготовку Alpine для **red.net** (DMZ) см. в шаге 5.

### Шаг 2: Сборка топологии в GNS3

```text
[Tiny Core PC1, PC2] → Switch1 → ether1 (MikroTik)   — blue.net (клиенты)
[Alpine DNS]         → Switch2 → ether2 (MikroTik)   — red.net  (DMZ)
```

* **Устройства:** MikroTik, 2× Ethernet Switch, 2× Tiny Core (QEMU), 1× Alpine (VirtualBox) с BIND9.
...
### Шаг 4: Настройка клиентов (blue.net, Tiny Core)

**PC1** — в `/opt/bootlocal.sh`:

```bash
pkill udhcpc
ifconfig eth0 172.16.10.1 netmask 255.255.255.0 broadcast 172.16.10.255 up
route add default gw 172.16.10.254
```

В `/etc/resolv.conf` укажите DNS из **red.net**:

```text
nameserver 172.16.20.53
```

Сохраните `resolv.conf`: добавьте путь в `/opt/.filetool.lst` и выполните `filetool.sh -b`.

**PC2:** то же с IP `172.16.10.2`.

### Шаг 5: Настройка DMZ — red.net (Alpine + BIND9)
...
### Шаг 6: Проверка с клиента (blue.net)
...
### Шаг 7: Firewall на MikroTik (blue.net ↔ red.net)
